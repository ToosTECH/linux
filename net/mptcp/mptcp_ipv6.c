				  ...
				  u16 queue_mapping)
{
	...
	fl6.fl6_dport = treq->ir_rmt_port;
	fl6.fl6_sport = treq->ir_num;
	...
	if (skb) {
		__tcp_v6_send_check(skb, &treq->ir_v6_loc_addr, &treq->ir_v6_rmt_addr);
				      ...
				      struct dst_entry *dst)
{
	...
	tcp_synack_rtt_meas(newsk, tcp_rsk(req)->snt_synack);
	...
	if (!isn) {
		...
		/* Kill the following clause, if you dislike this way. */
			...
			LIMIT_NETDEBUG(KERN_DEBUG "TCP: drop open request from %pI6/%u\n",
				       &treq->ir_v6_rmt_addr, ntohs(tcp_hdr(skb)->source));
			goto drop_and_release;
		}
